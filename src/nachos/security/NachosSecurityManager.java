...
public class NachosSecurityManager extends SecurityManager {
	...
	public Privilege getPrivilege() {
		Lib.assert_(this != System.getSecurityManager());
	...
	private void enablePrivilege() {
		if (privilegeCount == 0) {
			Lib.assert_(privileged == null);
			privileged = Thread.currentThread();
			privilegeCount++;
		} else {
			Lib.assert_(privileged == Thread.currentThread());
			privilegeCount++;
		}
	}
	...
	private void disablePrivilege() {
		Lib.assert_(privileged != null && privilegeCount > 0);
		privilegeCount--;
		if (privilegeCount == 0)
			privileged = null;
	}
	...
	public void checkPermission(Permission perm) {
		...
		// Need to allow privileged threads to reflect on classes
