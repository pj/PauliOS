...
public final class Coff {
	 ...
	 */
	public Coff(byte[] input) throws CoffLoadException {
		this.file = input;
		...
		System.arraycopy(input, 0, headers, 0, headers.length);
//
//		Lib.strictReadFile(file, 0, headers, 0, headers.length);
		...
		for (int s = 0; s < numSections; s++) {
			...
			sections[s] = new CoffSection(input, sectionEntryOffset);
		}
	}
	...
	public CoffSection getSection(int sectionNumber) {
		Lib.assert_(sectionNumber >= 0 && sectionNumber < sections.length);
